...
export default function CollectionManager({ collections: initial, visitedSpaces }: Props) {
  ...
  async function removeSpace(collectionId: string, spaceId: string) {
    ...
    setCollections((prev) =>
      prev.map((c) =>
        c.id === collectionId
          ? { ...c, items: c.items.filter((i) => i.spaceId !== spaceId) }
          : c
      )
    );
  }
  ...
  return (
    <div className="space-y-4">
      {collections.map((col) => {
        const available = visitedSpaces.filter(
          (s) => !col.items.some((i) => i.spaceId === s.id)
        );
        return (
              ...
              {editingId === col.id ? (
                ...
                <>
                  <p className="text-xs flex-1" style={{ color: "var(--fg)" }}>
                    {col.name}
                    <span style={{ color: "var(--dim)" }}> ({col.items.length})</span>
                  </p>
                  <button
              ...
              <div className="space-y-1 pl-2">
                {col.items.map((item) => (
                  <div key={item.spaceId} className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => removeSpace(col.id, item.spaceId)}
                      className="text-xs"
                      style={{ color: "var(--dim)" }}
                    >
                      −
              ...
              <div className="pl-2 space-y-1">
                {available.length === 0 ? (
                  ...
                  available.map((s) => (
                    <button
                      key={s.id}
                      onClick={() => addSpace(col.id, s.id)}
                      className="block text-xs w-full text-left py-0.5"
                      style={{ color: "var(--dim)" }}
                    >
                      + {s.name}
                    </button>

...
export default function MaterialsManager({ baseUrl, initialMaterials }: Props) {
  ...
  function publicUrl(s: string): string {
    return `${baseUrl}/materials/${s}.pdf`;
  }
  ...
  async function handleUpload() {
    ...
    if (!normalizedSlug) {
      showToast("링크 이름(영문)을 입력해주세요. 예: install-guide");
      return;
    }
    if (!file) {
      ...
      <div className="p-4 border space-y-3" style={{ borderColor: "var(--border)" }}>
        ...
        <div className="space-y-1">
          <input
            ...
            placeholder="링크 이름(영문, 예: install-guide)"
            className="w-full px-3 py-2 text-sm border bg-transparent"
            style={{ borderColor: "var(--border)" }}
          />
          {slug && (
            <p className="text-xs break-all" style={{ color: "var(--dim)" }}>
              → {publicUrl(normalizeSlug(slug) || "…")}
            </p>
          )}
        </div>
        <input
          ...
          {materials.map((m) => (
            <div key={m.id} className="p-4 border space-y-3" style={{ borderColor: "var(--border)" }}>
              ...
              <div className="flex gap-3 text-xs flex-wrap items-center">
                <button
                  onClick={() => copyLink(publicUrl(m.slug))}

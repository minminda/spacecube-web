...
export default async function SpacePage({ params }: Props) {
  ...
  const t = {
    location:      lang === "ko" ? "위치" : lang === "ja" ? "場所" : "Location",
    hours:         lang === "ko" ? "운영" : lang === "ja" ? "営業時間" : "Hours",
    storyNote:     lang === "ko" ? "이 공간의 이야기는 공간에서 열립니다." : lang === "ja" ? "この空間のストーリーは現地で始まります。" : "The story of this space unfolds within.",
    lovedBy:       lang === "ko" ? "이 공간을 좋아한 사람들" : lang === "ja" ? "この空間が好きな人たち" : "People Who Loved This Space",
    viewTaste:     lang === "ko" ? "이 취향 보기 →" : lang === "ja" ? "この好みを見る →" : "View taste →",
    editRecord:    lang === "ko" ? "[[ 기록 수정하기 ]]" : lang === "ja" ? "[[ 記録を編集する ]]" : "[[ Edit Record ]]",
    leaveRecord:   lang === "ko" ? "[[ 이 공간, 기록 남기기 ]]" : lang === "ja" ? "[[ この空間を記録する ]]" : "[[ Leave a Record ]]",
    signInRecord:  lang === "ko" ? "[[ 로그인하고 기록 남기기 ]]" : lang === "ja" ? "[[ ログインして記録する ]]" : "[[ Sign In to Leave a Record ]]",
  };
    ...
    <main className="flex flex-col min-h-screen">
      ...
      <div className="flex flex-col gap-6 px-6 py-6 flex-1">
        ...
        <div className="space-y-2">
          ...
          {space.tagline && (
            <p className="text-sm italic" style={{ color: "var(--dim)" }}>&ldquo;{space.tagline}&rdquo;</p>
          )}
        </div>

        <div className="space-y-1 text-xs" style={{ color: "var(--dim)" }}>
          {space.naverMapUrl ? (
            <a href={space.naverMapUrl} target="_blank" rel="noopener noreferrer"
              className="block hover:underline" style={{ color: "var(--dim)" }}>
                ...
                return (
                  <div key={r.id} className="space-y-1">
                    <div className="flex items-baseline justify-between">
                      ...
                      <Link href={`/u/${r.user.id}`} className="text-xs hover:underline flex-shrink-0 ml-3"
                        style={{ color: "var(--dim)" }}>
                        {t.viewTaste}
                      </Link>
                    </div>

    ...
    <main className="flex flex-col min-h-screen px-6 py-8 gap-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: "var(--muted)" }}>기억에 남는 느낌은?</p>
        </div>
      </div>

      <div>
        <p className="text-sm mb-4" style={{ color: "var(--muted)" }}>
          지금 이 공간에서 느끼는 감정을 골라봐.
        </p>
      ...
      <div>
        <p className="text-sm mb-2" style={{ color: "var(--muted)" }}>한 줄 메모 (선택)</p>
        <textarea
          value={memo}
          onChange={(e) => setMemo(e.target.value.slice(0, 100))}
          placeholder="오늘 여기서 무언가를 느꼈다면..."
          rows={3}
          className="w-full text-sm rounded-2xl p-4 resize-none outline-none"
          style={{
      ...
      <div className="mt-auto">
        <button
          ...
          className="w-full py-3 rounded-full text-sm transition-opacity disabled:opacity-30"

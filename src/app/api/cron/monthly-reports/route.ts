 ...
 * 관리자 페이지(/admin/[id]/report)에서 ReportEmail(실제 메일과 동일한 컴포넌트)로 그대로
...
export async function POST(req: NextRequest) {
  ...
  // 리포트는 관리자 페이지(/admin/[id]/report)에서 수동으로 생성·확인할 수 있다.
  ...
  for (const space of spaces) {
    ...
    for (const period of completedPeriods) {
      ...
      pending.push({
        ...
        url: `/admin/${space.id}/report?reportId=${report.id}`,
      });
    }
  }

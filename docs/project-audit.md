> ⚠️ **2026-07-26 재감사 결과 이 문서는 상당 부분 stale합니다.** 운영자 PIN 인증 도입, KPI/리포트
> 통합, 도메인·큐브코드 체계 변경 등으로 아래 서술 다수가 더 이상 사실과 다릅니다 — 특히
> `src/lib/operatorAuth.ts`/`requireSpaceAccess`(§5, §7, §9에서 언급)는 현재 코드베이스에
> **존재하지 않으며**, `/api/operator/spaces` 계열 API도 폴더 자체가 삭제되었습니다. 정리 작업의
> 최신 근거는 이 문서가 아니라 `docs/project-audit-2026-07-26.md`를 사용하세요. 아래 내용은
> 2026-07-14 시점 스냅샷으로 히스토리 보존 목적으로만 남깁니다.
